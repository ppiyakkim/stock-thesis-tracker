...
@media(max-width:640px){{
  ...
  .price-stat-row{{gap:8px}}
  .pstat{{padding:5px 9px}}
  .pstat-value{{font-size:12px}}
}}
@media(max-width:400px){{
  .fgrid{{grid-template-columns:1fr!important}}
  .cg{{flex-wrap:wrap}}
  .chart-wrap{{height:220px}}
  .vol-wrap{{height:55px}}
  .pstat-value{{font-size:11px}}
}}

/* ── Embed mode ── */
body.embed-mode{{background:transparent;overflow:hidden}}
body.embed-mode html{{overflow:hidden}}
body.embed-mode .content{{padding:0;height:100vh;overflow:hidden}}
body.embed-mode .month-header{{display:none}}
body.embed-mode .panel.visible{{
  display:grid;
  grid-template-rows:auto auto auto auto auto 1fr auto;
  height:100vh;overflow:hidden;gap:0}}
/* card-header: compact */
body.embed-mode .card-header{{padding:6px 10px 4px;flex-shrink:0}}
body.embed-mode .ticker-badge{{font-size:13px}}
body.embed-mode .label-text{{font-size:11px}}
body.embed-mode .meta-item{{font-size:9px}}
body.embed-mode .updated{{display:none}}
body.embed-mode .embed-toggle-btn{{display:none}}
/* tags: one line, tiny */
body.embed-mode .tag-row{{padding:0 10px;margin-bottom:2px;flex-shrink:0}}
body.embed-mode .tag-pill{{font-size:9px;padding:1px 6px}}
/* thesis: hidden in embed (saves space) */
body.embed-mode .thesis-text{{display:none}}
/* price stats: single compact row */
body.embed-mode .price-stat-row{{
  padding:2px 10px;margin-bottom:2px;flex-shrink:0;gap:6px}}
body.embed-mode .pstat{{padding:3px 8px}}
body.embed-mode .pstat-label{{font-size:8px}}
body.embed-mode .pstat-value{{font-size:12px}}
/* embed bar: hidden */
body.embed-mode .embed-bar{{display:none!important}}
/* sliders: single compact row */
body.embed-mode .ctrl-row{{
  padding:4px 10px;margin-bottom:2px;gap:10px;flex-shrink:0}}
body.embed-mode .cg label{{font-size:9px}}
body.embed-mode .cg input[type=range]{{width:90px}}
body.embed-mode .cv{{font-size:10px}}
/* legend: compact */
body.embed-mode .leg-row{{padding:0 10px;margin-bottom:3px;flex-shrink:0;gap:8px}}
body.embed-mode .leg{{font-size:9px}}
/* chart: fills ALL remaining space */
body.embed-mode .chart-wrap{{
  flex:1;height:auto!important;min-height:0;
  border-radius:0;overflow:hidden}}
/* volume: fixed small strip */
body.embed-mode .vol-wrap{{
  height:52px!important;flex-shrink:0;border-radius:0}}
@media(max-width:640px){{
  body.embed-mode .price-stat-row{{flex-wrap:nowrap;overflow-x:auto}}
  body.embed-mode .ctrl-row{{gap:6px}}
  body.embed-mode .cg input[type=range]{{width:60px}}
  body.embed-mode .chart-wrap{{min-height:120px}}
  body.embed-mode .vol-wrap{{height:40px!important}}
}}
</style>
</head>
<body>
...
function switchMonth(month) {{
  ...
  document.querySelectorAll('.tab').forEach(t => {{
    ...
    if (inMonth && inTag && firstVisible === -1)
      firstVisible = parseInt(t.id.replace('tab-',''));
  }});
...
document.addEventListener('DOMContentLoaded', () => {{
  ...
  if (embedMode) {{
    ...
    // Auto-resize Plotly when iframe / container resizes
  ...
  // Pick initial tab from URL or first tab in active month
  let startTab = -1;
  if (tickerParam) {{
  ...
  if (startTab < 0) {{
    const firstTab = document.querySelector('.tab:not(.month-hidden):not(.tag-hidden)');
    if (firstTab) startTab = parseInt(firstTab.id.replace('tab-',''));
  }}
  if (startTab >= 0) switchTab(startTab);
}});
...
function filterTag(tag) {{
  ...
  const btn = document.getElementById(tag ? 'tagbtn-'+tag : 'tagbtn-all');
  ...
  document.querySelectorAll('.panel').forEach(p => {{
    const i    = parseInt(p.id.replace('panel-',''));
    const tags = (p.dataset.tags || '').split(' ').filter(Boolean);
    const show = !tag || tags.includes(tag);
    const tab  = document.getElementById('tab-'+i);
    if (tab) tab.classList.toggle('tag-hidden', !show);
    if (show) {{
      visibleMonths.add(p.dataset.month);
      if (needSwitch && p.classList.contains('visible')) needSwitch = false;
    }}
    if (!show && p.classList.contains('visible')) {{
      // current panel is being hidden — switch to first visible
    }}
  }});

  // If current tab got hidden, switch to first visible
  if (needSwitch || document.querySelector('.panel.visible .tag-hidden') !== null) {{
    const firstVisible = document.querySelector('.tab:not(.tag-hidden):not(.month-hidden)');
    if (firstVisible) {{
      const idx = parseInt(firstVisible.id.replace('tab-',''));
      switchTab(idx);
    }}
  }}
  // Also check if current visible panel is now tag-hidden
  const curPanel = document.querySelector('.panel.visible');
  if (curPanel) {{
    const curI   = parseInt(curPanel.id.replace('panel-',''));
    const curTab = document.getElementById('tab-'+curI);
    if (curTab && curTab.classList.contains('tag-hidden')) {{
      const firstVisible = document.querySelector('.tab:not(.tag-hidden):not(.month-hidden)');
      if (firstVisible) switchTab(parseInt(firstVisible.id.replace('tab-','')));
    }}
  }}
...
function switchTab(i) {{
  // Use id-based lookup — immune to DOM order
  document.querySelectorAll('.tab').forEach(t => {{
    t.classList.toggle('active', t.id === 'tab-'+i);
  }});
  document.querySelectorAll('.panel').forEach(p => {{
    p.classList.toggle('visible', p.id === 'panel-'+i);
    p.classList.toggle('hidden',  p.id !== 'panel-'+i);
  }});
  // Update URL param without page reload
  const params = new URLSearchParams(location.search);
  if (STOCKS[i]) params.set('ticker', STOCKS[i].ticker);
