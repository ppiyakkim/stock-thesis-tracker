...
// ── Ticker search: Yahoo JSONP → dumbstockapi fallback ────────────────────────
const searchTimers = {{}};
let _jsonpIdx = 0;
...
function tryYahoo(i, q, sr) {{
  const cbName = '__yq_' + (++_jsonpIdx);
  const prev   = document.getElementById('yq-script');
  if (prev) prev.remove();

  // 3 sec timeout → fallback to dumbstockapi
  ...
  window[cbName] = function(data) {{
    ...
    const items = (data?.ResultSet?.Result || [])
      .filter(r => r.type === 'S' || r.type === 'E');
    if (!items.length) {{ tryDumb(i, q, sr); return; }}
    sr.innerHTML = items.slice(0, 10).map(r => {{
      const sym  = (r.symbol   || '').replace(/'/g, "\\'");
      const name = (r.name     || '').replace(/'/g, "\\'").slice(0, 45);
      const exch = (r.exchDisp || r.exch || '').replace(/'/g, "\\'");
      const type = r.typeDisp  || '';
      return `<div class="search-item" onclick="selectTicker(${{i}},'${{sym}}','${{name}}','${{exch}}')">
        ...
        <span class="si-exch">${{exch}}${{type ? ' · '+type : ''}}</span>
      </div>`;
    }}).join('');
  }};
  ...
  s.id     = 'yq-script';
  s.src    = 'https://autoc.finance.yahoo.com/autoc?query='
    + encodeURIComponent(q) + '&region=1&lang=en&callback=' + cbName;
  s.onerror = () => {{ clearTimeout(timer); delete window[cbName]; tryDumb(i, q, sr); }};
  document.head.appendChild(s);
}}

async function tryDumb(i, q, sr) {{
  try {{
